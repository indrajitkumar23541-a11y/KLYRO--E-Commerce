...
import { Bell, ShoppingBag, Search, User, Menu, X, ChevronDown, Heart, ArrowRight, Car, Sparkles, Book, Smartphone, Leaf, Armchair, Dumbbell, Tag, Moon, Settings, ChevronRight, Zap, Package, Briefcase, LayoutGrid, Baby } from 'lucide-react';
...
const Navbar = () => {
    ...
    const mainNavItems = [
        { id: 'fashion', name: "Fashion", path: "/fashion", icon: <ShoppingBag size={14} />, keywords: ['fashion', 'men', 'women', 'footwear', 'watch', 'jewelry'] },
        { id: 'electronics', name: "Electronics", path: "/electronics", icon: <Smartphone size={14} />, keywords: ['electronics', 'smartphone', 'laptop', 'wearable', 'audio', 'gaming', 'smart home', 'office tech'] },
        { id: 'beauty-health', name: "Beauty & Health", path: "/beauty-health", icon: <Sparkles size={14} />, keywords: ['beauty', 'health', 'skincare', 'makeup', 'haircare', 'fragrance', 'grooming'] },
        { id: 'home-living', name: "Home & Living", path: "/home-living", icon: <Armchair size={14} />, keywords: ['home', 'living', 'furniture', 'decor', 'bedding', 'kitchen'] },
        { id: 'automotive', name: "Automotive", path: "/automotive", icon: <Car size={14} />, keywords: ['automotive', 'car', 'bike'] },
        { id: 'sports-fitness', name: "Sports & Fitness", path: "/sports-fitness", icon: <Dumbbell size={14} />, keywords: ['sports', 'fitness', 'cycling', 'yoga', 'indoor sports'] },
        { id: 'grocery', name: "Grocery", path: "/grocery", icon: <Leaf size={14} />, keywords: ['grocery', 'food', 'snack'] },
        { id: 'books-education', name: "Books & Education", path: "/books-education", icon: <Book size={14} />, keywords: ['books', 'education', 'stationery', 'art'] },
        { id: 'kids-baby', name: "Kids & Baby", path: "/kids-baby", icon: <Baby size={14} />, keywords: ['kids', 'baby', 'toy'] },
        { id: 'others', name: "Others", path: "/others", icon: <LayoutGrid size={14} />, keywords: ['others', 'pet', 'musical'] }
    ];
        ...
        const fetchCategories = async () => {
            try {
                ...
                if (response.data.success) {
                    setCategoryTree(buildCategoryTree(response.data.categories));
                }
            } catch (error) {
    ...
    return (
        <>
            <header className="fixed top-0 left-0 w-full z-[100] shadow-2xl animate-reveal">
                    ...
                    <div className="ml-auto flex items-center gap-1 sm:gap-6 lg:gap-7 text-white z-20">
                        ...
                        {user && (
                            <button 
                                ...
                                className="relative w-10 h-10 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-white transition-all border border-white/5"
                            >
                                <Bell size={20} strokeWidth={2.5} />
                                {unreadCount > 0 && (
                                    <span className="absolute -top-1 -right-1 bg-blue-600 text-white text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center shadow-lg ring-1 ring-blue-600/30">
                                        {unreadCount}
                                    </span>
                                )}
                            </button>
                ...
                {/* BOTTOM ROW: Categories (Hidden on Category Pages to avoid duplicate sub-navbars) */}
                {!['/fashion', '/electronics', '/beauty-health', '/home-living', '/automotive', '/sports-fitness', '/grocery', '/books-education', '/kids-baby', '/others'].some(path => location.pathname.startsWith(path)) && (
                    <nav className="header-bottom h-12 hidden md:flex items-center relative shadow-inner overflow-x-auto no-scrollbar w-full bg-[#1e40af] border-b border-white/10">
                        <div className="flex h-full min-w-max mx-auto px-4 xl:px-12">
                            {mainNavItems.map((item) => {
                                // Find relevant root categories from the dynamic tree for the mega menu
                                const relevantRoots = categoryTree.filter(root => 
                                    item.keywords.some(kw => root.name.toLowerCase().includes(kw))
                                );
                                ...
                                return (
                                    ...
                                    >
                                        <Link
                                            to={item.path}
                                            onClick={closeMenu}
                                            className={`h-full flex items-center gap-2 px-4 xl:px-6 text-[10px] xl:text-[11px] font-black uppercase tracking-[0.15em] transition-all whitespace-nowrap relative group ${activeDropdown === item.id ? 'text-white bg-white/10' : 'text-white/80 hover:text-white'}`}
                                        >
                                            <span className="opacity-60 group-hover:opacity-100 transition-opacity">
                                                {item.icon}
                                            </span>
                                            {item.name}
                                            <span className={`absolute bottom-0 left-0 w-full h-[3px] bg-white transition-all duration-300 ${activeDropdown === item.id ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0 group-hover:opacity-50 group-hover:scale-x-50'}`} />
                                        </Link>

                                        {/* MEGA MENU: Active Dropdown Logic */}
                                        {relevantRoots.length > 0 && activeDropdown === item.id && (
                                            <div className="absolute top-full left-1/2 -translate-x-1/2 pt-0 w-[1100px] max-w-[95vw] animate-reveal opacity-100 visible z-[150]">
                                                <div className="mega-menu-triangle !border-b-white"></div>
                                                <div className="bg-white shadow-[0_30px_100px_rgba(0,0,0,0.3)] rounded-b-3xl p-12 flex border-t-4 border-blue-600 cursor-default" onMouseEnter={(e) => e.stopPropagation()}>
                                                    <div className="grid grid-cols-4 gap-10 w-full">
                                                        {relevantRoots.slice(0, 3).map((root, rootIdx) => (
                                                            <div key={root.id} className="space-y-6">
                                                                <Link to={`/products?category_id=${root.id}`} onClick={closeMenu} className="text-[13px] font-black uppercase text-blue-900 border-b-2 border-blue-50 pb-3 block tracking-widest hover:text-blue-600 transition-colors">
                                                                    {root.name}
                                                                </Link>
                                                                <ul className="space-y-3">
                                                                    {root.children?.slice(0, 8).map(child => (
                                                                        <li key={child.id}>
                                                                            <Link to={`/products?category_id=${child.id}`} onClick={closeMenu} className="flex items-center text-[12px] text-slate-500 hover:text-blue-600 font-bold hover:translate-x-1 transition-all">
                                                                                <span className="w-1.5 h-1.5 rounded-full bg-blue-100 mr-2 group-hover:bg-blue-600"></span> {child.name}
                                                                            </Link>
                                                                        </li>
                                                                    ))}
                                                                </ul>
                                                        ...
                                                        <div className="space-y-6 flex flex-col items-center">
                                                            <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl group/img">
                                                                <img src={`/assets/${item.id}_hero.webp`} alt="Highlight" className="w-full h-full object-cover group-hover/img:scale-110 transition-transform duration-700" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&q=80&w=400" }} />
                                                                <div className="absolute inset-0 bg-gradient-to-t from-blue-900/80 to-transparent"></div>
                                                                <div className="absolute bottom-6 left-6 text-white">
                                                                    <p className="text-[9px] font-black uppercase tracking-[0.2em] text-blue-200 mb-1">Featured Selection</p>
                                                                    <p className="text-lg font-black uppercase leading-tight">Explore {item.name}</p>
                                                                </div>
                                                            </div>
                                                            <div className="w-full bg-blue-50 p-4 rounded-xl flex items-center justify-between group cursor-pointer hover:bg-blue-100 transition-colors" onClick={() => navigate(item.path)}>
                                                                <span className="text-[10px] font-black uppercase text-blue-900 tracking-widest">View Full Hub</span>
                                                                <ArrowRight size={14} className="text-blue-600 group-hover:translate-x-1 transition-all" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                            ...
                            <div className="space-y-6 relative">
                                ...
                                <div className="space-y-4">
                                    {[
                                        ...
                                        { name: "Kids & Baby", icon: <Baby size={20} />, path: '/kids-baby' },
                                        { name: "Others", icon: <LayoutGrid size={20} />, path: '/others' }
                                    ].map((cat, idx) => (
                                        <Link
                                            key={idx}
                                            to={cat.path}
