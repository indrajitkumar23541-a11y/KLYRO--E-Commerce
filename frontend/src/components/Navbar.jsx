...
import { ShoppingBag, Search, User, Menu, X, ChevronDown, Heart, ArrowRight, Car, Sparkles, Book, Smartphone, Leaf, Armchair, Dumbbell, Tag, Moon, Settings, ChevronRight } from 'lucide-react';
...
const Navbar = () => {
    ...
    return (
        <>
        <header className="fixed top-0 left-0 w-full z-[100] shadow-xl animate-reveal">
            {/* TOP ROW: Logo, Search, User Icons */}
            <div className="header-top h-[64px] md:h-[72px] flex items-center px-4 md:px-8 lg:px-12 gap-2 md:gap-8 bg-[#0a0a0b]/95 backdrop-blur-md border-b border-white/5 relative">
                
                {/* Mobile Menu Toggle (Left) */}
                <button onClick={() => setIsMenuOpen(true)} className="lg:hidden w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/10 transition-colors z-20">
                    <Menu size={22} className="text-white" />
                </button>

                {/* Logo (Centered on Mobile, Left on Desktop) */}
                <div className="absolute left-1/2 -translate-x-1/2 lg:static lg:translate-x-0 z-10">
                    <Link to="/" onClick={closeMenu} className="text-xl md:text-2xl font-black uppercase tracking-[0.2em] text-white whitespace-nowrap group font-serif-premium">
                        KLYRO
                    </Link>
                </div>

                {/* Search Bar (Desktop) */}
                <div className="flex-grow hidden md:flex justify-center max-w-4xl mx-auto pl-12">
                    <form onSubmit={handleSearch} className="w-full relative flex shadow-2xl group">
                        <input 
                            type="text" 
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="w-full bg-white/5 h-11 px-6 rounded-l-xl rounded-r-none text-sm outline-none text-white placeholder:text-gray-500 focus:bg-white/10 border border-white/10 focus:border-[#717fe0]/50 transition-all"
                            placeholder="Discover the extraordinary..."
                        />
                        <button type="submit" className="bg-[#717fe0] hover:bg-[#5b6bc0] h-11 px-8 flex items-center justify-center text-white transition-all rounded-r-xl font-bold uppercase text-[10px] tracking-widest">
            ...
            <nav className="header-bottom h-12 hidden lg:flex items-center relative shadow-inner overflow-x-auto no-scrollbar w-full">
                <div className="flex h-full min-w-max mx-auto px-4 xl:px-8">
                    {categoryTree.map((parent) => (
                        ...
                        >
                            <Link 
                                ...
                                onClick={closeMenu}
                                className={`h-full flex items-center gap-1.5 xl:gap-2 px-3 lg:px-4 xl:px-6 text-[11px] xl:text-[12px] font-black uppercase tracking-tighter transition-all whitespace-nowrap ${activeDropdown === parent.id ? 'text-white bg-white/10' : 'text-white/80 hover:text-white'}`}
                            >
                                {parent.name}
                                {parent.children.length > 0 && <ChevronDown size={14} className={`opacity-50 transition-transform ${activeDropdown === parent.id ? 'rotate-180' : ''}`} />}
                            </Link>
                ...
                <div className="absolute left-0 top-0 w-[85%] max-w-[380px] h-full bg-[#1a1a1c] animate-reveal-left flex flex-col shadow-[20px_0_60px_rgba(0,0,0,0.9)] overflow-hidden" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="relative p-8 pb-6 z-10">
                        <div className="flex justify-between items-start mb-6">
                            <div className="w-16 h-16 rounded-full border-2 border-[#717fe0]/40 overflow-hidden shadow-2xl">
                                <img 
