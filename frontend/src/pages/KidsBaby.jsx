...
import { ShoppingBag, ChevronRight, ArrowRight, Heart, Sparkles, ShieldCheck, Truck, Award, Headphones, X } from 'lucide-react';
import KidsBabyData from '../data/KidsBabyData';
...
const KidsBaby = () => {
    ...
    useEffect(() => {
        document.title = "KLYRO | Kids & Baby";
        window.scrollTo(0, 0);
        fetchMetadata();
    }, []);
    ...
    const fetchMetadata = async () => {
        try {
            ...
            // Dynamically find Kids & Baby root
            const root = raw.find(c => c.name.toLowerCase().includes('kids'));
            if (root) {
            ...
            console.error('Kids metadata fetch error:', error);
        }
    };
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            console.error('Kids products fetch error:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="bg-[#fdfaf5] min-h-screen pt-[56px] md:pt-[64px] pb-28 md:pb-24 page-transition font-sans">
            ...
            <section className="mb-8 md:mb-16 w-full animate-reveal relative group px-0 md:px-6 lg:px-0">
                <div className="relative h-[40vh] md:h-[50vh] min-h-[350px] md:min-h-[450px] w-full overflow-hidden shadow-sm md:rounded-3xl lg:rounded-none border-b border-orange-50 bg-white">
                    ...
                    <div 
                        ...
                        style={{ backgroundImage: `url(/assets/kids_hero_v2.png)` }}
                    />
                    ...
                    <div className="relative z-20 h-full flex flex-col justify-center px-6 md:px-12 lg:px-24 max-w-[1440px] mx-auto space-y-4 md:space-y-6">
                        ...
                        <div className="space-y-2 md:space-y-4">
                            <h1 className="text-4xl sm:text-6xl lg:text-8xl font-black text-[#431407] tracking-tight leading-[0.9]">
                                Little <span className="text-[#bc612c]">Wonders</span>
                            </h1>
                            <p className="text-sm md:text-xl lg:text-2xl font-bold text-slate-600 tracking-tight leading-relaxed max-w-lg mb-4 md:mb-8 uppercase">
                                Pure Joy for Your Little Explorers.
                            </p>
                            <div className="hidden sm:flex flex-wrap gap-2 md:gap-4 pt-2">
                                {['Safe & Tested', 'Educational Play', 'Premium Quality', 'Happy Parenting'].map((bullet, i) => (
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 md:gap-10 pt-2 md:pt-4">
                            <button 
                                ...
                                className="bg-[#bc612c] hover:bg-[#a0522d] text-white px-8 md:px-12 py-3 md:py-4 rounded-md font-black text-[10px] md:text-xs transition-all shadow-lg active:scale-95 uppercase tracking-widest"
                            >
                                Shop Little Wonders &rarr;
                            </button>
                            <button 
                                onClick={() => navigate(`/products?category_id=${rootCategoryId}&sort=popularity`)}
                                className="text-[#431407] hover:text-[#bc612c] font-black text-[10px] md:text-xs transition-all uppercase tracking-widest flex items-center gap-2 group/btn"
                            >
            ...
            <section className="bg-gray-50/50 py-16 md:py-24 mb-12 md:mb-20 animate-reveal stagger-1">
                <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-12">
                    <div className="flex items-center justify-between mb-12 border-b border-gray-200/50 pb-6">
                        <h2 className="text-2xl md:text-3xl font-black text-slate-800 uppercase tracking-tight">Shop by Playtime</h2>
                        <button onClick={() => navigate('/products')} className="text-[10px] md:text-[12px] font-black uppercase tracking-widest text-[#bc612c] border-2 border-[#bc612c]/20 px-6 py-3 rounded-full hover:bg-[#bc612c] hover:text-white transition-all shadow-sm">
                            View All Categories
                        </button>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8">
                        {[
                            { name: "Toys", img: "/assets/kids/cat_toys.png" },
                            { name: "Clothing", img: "/assets/kids/cat_clothes.png" },
                            { name: "Baby Care", img: "/assets/kids/cat_care.png" },
                            { name: "Nursery", img: "/assets/home/cat_furnishing.png" },
                            { name: "Learning", img: "/assets/kids/cat_learning.png" },
                            { name: "Footwear", img: "/assets/kids/cat_footwear.png" }
                        ].map((cat, i) => (
                            <div 
                                key={i} 
            ...
            {/* EXCLUSIVE DEALS */}
            <section className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-12 mb-12 md:mb-24 animate-reveal">
                <div className="flex justify-between items-center mb-6 md:mb-8 border-b border-[#bc612c]/20 pb-4">
                    <h2 className="text-xl md:text-2xl font-bold text-slate-800">Exclusive Deals</h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
                    {KidsBabyData.deals.map((deal, i) => (
                        <div key={i} className="rounded-[30px] md:rounded-[40px] overflow-hidden relative shadow-xl h-[240px] md:h-[300px] group cursor-pointer">
                            ...
                            <div className={`absolute inset-0 ${deal.color} opacity-20 group-hover:opacity-10 transition-all z-0`} />
                            <div className="absolute inset-0 bg-gradient-to-r from-[#fdfaf5] via-[#fdfaf5]/60 to-transparent p-6 md:p-12 flex flex-col justify-center">
                                <h3 className="text-2xl md:text-4xl font-black text-slate-800 mb-2">{deal.title}</h3>
                                <p className="text-base md:text-xl font-bold text-red-600 mb-6 md:mb-8 uppercase tracking-widest italic">{deal.offer}</p>
            ...
            <section className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-12 mb-12 md:mb-20 animate-reveal stagger-3">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b border-[#bc612c]/20 pb-4 gap-4">
                    <h2 className="text-xl md:text-2xl font-bold text-slate-800">Bestsellers</h2>
                    <button onClick={() => navigate('/products?category_id=42')} className="text-[9px] md:text-[11px] font-semibold uppercase tracking-widest text-[#bc612c] border border-[#bc612c]/30 px-4 md:px-6 py-2.5 rounded-full hover:bg-white transition-colors w-fit">
                        View All &rsaquo;
                    </button>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                    {KidsBabyData.bestsellers.map((item) => (
                        <div key={item.id} className="bg-white rounded-[24px] md:rounded-[40px] p-4 md:p-6 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col border border-stone-50 h-full group">
                            <div className="h-44 md:h-64 flex items-center justify-center mb-4 md:mb-6 relative z-0 overflow-hidden rounded-[20px] md:rounded-[32px] bg-[#fdfaf5]">
                                ...
                                <button className="absolute top-2 right-2 md:top-4 md:right-4 p-2 md:p-3 bg-white/80 backdrop-blur-md rounded-full text-red-500 shadow-sm opacity-0 group-hover:opacity-100 transition-all">
                                    <Heart size={14} md={18} />
                                </button>
                            </div>
                            <div className="flex flex-col flex-grow text-center">
                                <h4 className="font-bold text-slate-700 text-[10px] md:text-sm leading-tight mb-2 md:mb-3 line-clamp-2 px-1">{item.name}</h4>
                                <div className="flex items-center justify-center gap-2 mb-4 md:mb-6">
                                    <span className="text-[#bc612c] font-black text-base md:text-xl">₹{item.price}</span>
                                    <span className="text-[10px] md:text-xs text-slate-300 line-through font-bold">₹{item.oldPrice}</span>
                                </div>
                                <button className="w-full bg-slate-900 text-white py-2.5 md:py-4 rounded-lg md:rounded-xl font-black text-[8px] md:text-[10px] uppercase tracking-widest mt-auto shadow-md hover:bg-[#bc612c] transition-all">Add to Cart</button>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <section className="bg-gradient-to-b from-white to-[#fdfaf5] py-12 md:py-20 px-4 md:px-6 lg:px-12 mb-12 md:mb-20 overflow-hidden">
                <div className="max-w-[1440px] mx-auto">
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-12 mb-12 md:mb-20 border-b border-[#bc612c]/20 pb-12 md:pb-16">
                        {[
                            { title: 'Parenting', sub: 'Expert Tips', icon: <Sparkles size={20} md={24} />, guide: KidsBabyData.expertTips[0] },
                            { title: 'Tested', sub: 'Safe & Non-Toxic', icon: <ShieldCheck size={20} md={24} /> },
                            { title: 'Partners', sub: 'Top Brands', icon: <Award size={20} md={24} /> },
                            { title: 'Support', sub: '24/7 Experts', icon: <Headphones size={20} md={24} /> }
                        ].map((item, i) => (
                    ...
                    <div className="flex overflow-x-auto no-scrollbar gap-4 pb-4">
                        {KidsBabyData.brands.map((brand, i) => (
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white rounded-[32px] md:rounded-[48px] w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col relative z-10 shadow-3xl animate-reveal border border-white">
                        <div className="p-6 md:p-10 border-b border-[#fdfaf5] bg-[#fdfaf5]/50 flex justify-between items-start">
                            ...
                            <button onClick={() => setSelectedGuide(null)} className="p-2 bg-white rounded-full text-[#bc612c] hover:text-[#9e5d3c] transition-colors shadow-sm border border-[#bc612c]/20"><X size={18} md={20} /></button>
                        </div>
            ...
            <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-12 text-center pb-12 md:pb-20 border-t border-stone-100 pt-12 md:pt-20">
                <button 
                  onClick={() => navigate('/products?category_id=42')}
                  className="bg-[#2d231b] hover:bg-[#bc612c] text-white px-8 md:px-16 py-4 md:py-6 rounded-xl md:rounded-2xl font-black text-xs md:text-sm uppercase tracking-widest shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-2 md:gap-3 mx-auto"
                >
